...
function setup() {
  ...
  invLine=createSprite(500,displayHeight/2-70,7000,3000);

  invLine.visible=false;

  Ambline=createSprite(105,displayHeight/2-110,5,50);
  Ambline2=createSprite(width/2-40,displayHeight/2-20,20,50);
  Ambline3=createSprite(width/2+150,displayHeight/2-110,5,50);
...
function draw() {
  ...
  textSize(60);
  fill("red");
  text("Nowadays there are a lot of people who are   ",100,170);
  text("not able to reach the hospital,due to traffic",110,290);
  textSize(50);
  fill("green");
  text("This is a model which can help ambulances ",100,400);
  text("to reach the hospital in time",350,500);
  ...
  if( gameState==="play"){
    ...
    for(var i=0;i<ambGroup.length;i++){
      if(ambGroup.get(i).isTouching(Ambline)){
        ambGroup.get(i).y+=10;
      }
    }
    ...
    for(var i=0;i<ambGroup.length;i++){
      if(ambGroup.get(i).isTouching(Ambline2)){
        ambGroup.get(i).y-=10;
        num=1;
      }
    }
    ...
    for(var i=0;i<ambGroup.length;i++){
      if(ambGroup.get(i).isTouching(Ambline3)){
        num=1;
      } 
  ...
  switch(num){
    ...
    case 4:
      ...
      case 5:
        ...
        rightGroup.setVelocityXEach(0)
        rightsideGroup.setVelocityXEach(0)
        right1sideGroup.setVelocityXEach(0)
        right2sideGroup.setVelocityXEach(0)

        topGroup.setVelocityYEach(0)
        topsideGroup.setVelocityYEach(0)

        //leftGroup.setVelocityXEach(10)
        leftsideGroup.setVelocityXEach(0)
        left1sideGroup.setVelocityXEach(0)
        left2sideGroup.setVelocityXEach(0)

        bottomGroup.setVelocityYEach(0);
        bottomsideGroup.setVelocityYEach(0);
      },10000);
...
function spawnAmbulance(){
  if(frameCount%977===0){
    amb=createSprite(30,displayHeight/2-110,20,20);
...
function spawnFromLeftSide(){
  if(frameCount%100===0){
    leftsidecar=createSprite(170,displayHeight/2-110,20,20);
...
function spawnFromLeft1Side(){
  if(frameCount%100===0){
    left1sidecar=createSprite(290,displayHeight/2-110,20,20);
...
function spawnFromLeft2Side(){
  if(frameCount%100===0){
    left2sidecar=createSprite(420,displayHeight/2-110,20,20);
...
function spawnFromBottom(){
  if(frameCount%100===0){
   ...
   // bottomcar=createSprite(displayWidth/2-50,displayHeight-270,20,20);
